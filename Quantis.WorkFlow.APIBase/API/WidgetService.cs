            ...
            using (OracleConnection con = new OracleConnection(_connectionstring))
            {
                using (OracleCommand cmd = con.CreateCommand())
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        {
                            ContractParty = (string)reader[0],
                            Contract = (string)reader[1],
                            IdKPI = Decimal.ToInt32((Decimal)reader[2]),
                            DescrizioneKPI = (string)reader[3],
                            Tipologia = (string)reader[4],
                            Frequenza = (string)reader[5],
                            Calcolo = (string)reader[6],
                            Fornitura = (string)reader[7],
                            Escalation = (string)reader[8],
                            ViloreLimiteAtteso = (string)reader[9],
                            Trend = (reader[10] == DBNull.Value) ? 0.0 : Decimal.ToDouble((Decimal)reader[10]),
                            GEN = (string)reader[11],
                            FEB = (string)reader[12],
                            MAR = (string)reader[13],
                            APR = (string)reader[14],
                            MAG = (string)reader[15],
                            GIU = (string)reader[16],
                            LUG = (string)reader[17],
                            AGO = (string)reader[18],
                            SEP = (string)reader[19],
                            OTT = (string)reader[20],
                            NOV = (string)reader[21],
                            DIC = (string)reader[22]
                        });
                    }
                }
            }           
